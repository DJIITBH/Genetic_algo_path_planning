...
def _fitness(chromosome, path_points):
    ...
    turn_penalty = _turn_penalty(chromosome, path_points)

    fitness = 1 / (length + 1 * turn_penalty)
...
def _turn_penalty(chromosome, path_points):
    active_points = [path_points[i] for i, gene in enumerate(chromosome) if gene == '1']

    if len(active_points) < 3:
        return 0  # No turns to evaluate

    penalty = 0
    for i in range(1, len(active_points) - 1):
        p1 = np.array(active_points[i - 1])
        p2 = np.array(active_points[i])
        p3 = np.array(active_points[i + 1])
        ...
        # Penalize sharp turns more
        penalty += (np.pi - angle) ** 2  # max when angle is 0 (U-turn)
